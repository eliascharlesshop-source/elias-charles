...
export function BoxHUD({ selectedItems, totalPrice, onRemoveItem }: BoxHUDProps) {
  ...
  return (
    <motion.div 
      className="bg-gradient-to-b from-white to-gray-50 border-2 border-gray-200 rounded-2xl p-6 sticky top-8 space-y-6"
    ...
    >
      ...
      <div className="space-y-2 max-h-72 overflow-y-auto">
        {selectedItems.length === 0 ? (
          ...
          selectedItems.map((selected, idx) => (
            <motion.div 
              key={idx} 
              className="flex items-start justify-between text-sm bg-white border border-gray-100 p-3 rounded-lg hover:border-gray-300 transition-colors"
            ...
            >
              ...
              <button
                ...
                className="ml-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded p-1 transition-colors flex-shrink-0"
      ...
      <div className="space-y-2">
        ...
        {selectedItems.length > 0 && (
          <button
            className="w-full py-2.5 px-4 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors font-medium"
          >
            Start Over
          </button>
        )}
      ...
      <div className="bg-blue-50 rounded-lg p-3 space-y-2">
        <p className="text-xs font-semibold text-blue-900 mb-2">✓ Box Perks</p>
